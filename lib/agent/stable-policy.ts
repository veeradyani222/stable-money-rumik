...
export function buildStableProjectPromptRules(): string {
  return [
    ...
    '',
    'Response pattern when applicable: acknowledge, say what you will check, call the tool, and summarize only the requested result in plain language. Do not add a next step unless the caller asked, the route explicitly requires it, or the tool result requires clarification.',
    ...
    '- payment.failed: reassure before policy, verify read access, call get_payment_reconciliation_status, explain booking-or-refund. Offer or create a ticket only when the caller asks, the tool result recommends escalation, or the issue cannot be resolved on voice.',
